DROP VIEW IF EXISTS t_19 CASCADE;
CREATE OR REPLACE VIEW t_19 AS 
...
WITH
 ...
 edit_start AS (
  ...
  FROM   edit_t_19 AS e, duration_class AS d
 ...
 end_detail AS (
  ...
  FROM   edit_t_19 AS et, edited_stations_2022 AS es
 ...
 start_end_detail AS ( 
  SELECT s.*, e.date_end, e.end_year, e.end_quarter, e.end_month, e.time_end,
         ...
         e.passholder_type, e.bike_id, e.bike_type, e.station_start_date,
         e.status
...
---------------------------------------------
DROP VIEW IF EXISTS trip_shared_bike CASCADE;
CREATE OR REPLACE VIEW trip_shared_bike AS 
...
WITH 
 ...
 --WHERE passholder_type = 'Null'
 --LIMIT 10;
 --WHERE t.duration < '24:00:00'
 --AND t.passholder_type <> 'Null'
